    ...
    <div className="flex flex-col items-center w-full mx-auto">
      {pathInfos.map((pathInfo) => (
        <div key={pathInfo.path} className="flex items-center gap-2 my-2 w-full">
          ...
          >
            {pathInfo.isEndpoint ? <ScrollText className="w-5 h-5 flex-shrink-0 mt-0.5" /> : <Folder className="w-5 h-5 flex-shrink-0" />}
            {pathInfo.isEndpoint ? (
                  ...
                  <div className="flex flex-row items-center gap-2 text-xs text-muted-foreground">
                    {pathInfo.createdAt && (
                      <div className="flex flex-row items-center">
                        ...
                        {pathInfo.createdAt.toLocaleDateString('ja-JP', { 
                          year: 'numeric', 
                          month: '2-digit', 
                          day: '2-digit' 
                        }).split('/').join('.')}
