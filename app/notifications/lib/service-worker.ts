 ...
 * - Safari PWA では Service Worker のインストールが完了しない（installing のまま）ケースがあるため、
 *   一定時間待っても active にならない場合は、installing/waiting 状態の registration でも
 *   PushManager が利用可能なら返す。
 *
 * この関数は "PushManager を使える Registration" を返す。
 */
