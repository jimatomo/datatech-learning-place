...
export default function QuizContent() {
  const quiz = new Quiz({
    ...
    tags: ["Snowflake", "String Functions", "Data Manipulation", "SQL"],
    ...
    options: {
      ...
      1: "REGEXP_EXTRACT(SENDER_EMAIL, '@([a-zA-Z0-9.-]+)\\.')",
    ...
    references: [
      { title: "String Functions - Snowflake Documentation", url: "https://docs.snowflake.com/en/sql-reference/functions/string-functions" },
      { title: "Regular Expression Functions - Snowflake Documentation", url: "https://docs.snowflake.com/en/sql-reference/functions/regexp" },
    ],
  });
  return quiz;
}
...
function QuizExplanation() {
  return (
    <div className="text-xs md:text-sm">
      <p className="py-2">
        正解は <strong className="text-emerald-500">REGEXP_EXTRACT関数</strong> です。
      </p>
      ...
      <ul className="list-disc pl-4">
        ...
        <li className="pb-2">
          <strong className="text-emerald-500">REGEXP_EXTRACT：</strong> 最も適切で効率的（正解）
          <CodeBlock 
            code={`REGEXP_EXTRACT(SENDER_EMAIL, '@([a-zA-Z0-9.-]+)\\.')`}
            showLineNumbers={false}
          />
    ...
    CUSTOMER_NAME,
    SENDER_EMAIL,
    REGEXP_EXTRACT(SENDER_EMAIL, '@([a-zA-Z0-9.-]+)\\.') AS DOMAIN_NAME
...
SELECT REGEXP_EXTRACT(SENDER_EMAIL, '@(.+)') AS FULL_DOMAIN;

-- ユーザー名部分（@より前）
SELECT REGEXP_EXTRACT(SENDER_EMAIL, '([^@]+)@') AS USERNAME;

-- 複数のドメイン部分を分割
SELECT SPLIT_PART(REGEXP_EXTRACT(SENDER_EMAIL, '@(.+)'), '.', 1) AS DOMAIN_NAME;`}
        showLineNumbers={false}
      />
      ...
      <ul className="list-disc pl-4">
        ...
        <li>REGEXP_EXTRACT は一度の実行で複雑な抽出が可能</li>
