...
export default function QuizContent() {
  const quiz = new Quiz({
    ...
    tags: ["Snowflake", "クエリプロファイル", "パフォーマンス監視", "Snowsight", "Query History"],
  ...
  return (
    <div className="text-xs md:text-sm">
      ...
      <div className="mt-4 p-4 bg-yellow-50 border-l-4 border-yellow-400">
        <p className="font-semibold text-yellow-800">パフォーマンス最適化のヒント：</p>
        <ul className="list-disc pl-4 text-yellow-700 text-xs">
          <li>適切な結合条件を設定してデカルト積を避ける</li>
          <li>重複排除が不要な場合はUNION ALLを使用する</li>
          <li>メモリ不足の場合はより大きなウェアハウスを使用する</li>
          <li>効率的なプルーニングのためデータの編成を最適化する</li>
        </ul>
      </div>
    </div>
  );
