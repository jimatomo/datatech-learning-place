...
- Keep "select the incorrect statement" as the default because it efficiently tests whether the reader can spot a false claim among plausible truths. Occasionally vary the format when that variation creates a clearer hook and higher learning value. Variation is not limited to swapping "pick one incorrect" for "pick one correct" or a scenario prompt. Include changes in how many answers to select and how many options are shown, for example: five options; selecting two incorrect (or two correct) statements; a scenario-based choice; or a result prediction. Do not vary every quiz for its own sake. Within a multi-day batch, avoid making every question the same single-select incorrect-one pattern; use a different selection count or option count on at least one question when the topic supports it.
